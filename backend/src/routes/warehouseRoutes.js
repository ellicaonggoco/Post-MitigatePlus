...
router.post('/:id/restock', protect, requireRole('lgu_admin', 'lgu_superadmin'), async (req, res) => {
  try {
    ...
    await item.save();
    await WarehouseLog.create({ itemId: item._id, action: 'restock', quantity: Number(quantity), notes: notes || '', performedBy: req.user._id });
...
router.post('/:id/dispatch', protect, requireRole('lgu_admin', 'lgu_superadmin', 'field_staff'), async (req, res) => {
  try {
    ...
    const item = await WarehouseItem.findById(req.params.id);
    if (!item) return res.status(404).json({ message: 'Item not found' });
    if (item.stock < Number(quantity)) return res.status(400).json({ message: `Insufficient stock. Available: ${item.stock} ${item.unit}` });
    item.stock -= Number(quantity);
    item.updatedBy = req.user._id;
    await item.save();
    await WarehouseLog.create({ itemId: item._id, action: 'dispatch', quantity: Number(quantity), notes: notes || '', performedBy: req.user._id });
